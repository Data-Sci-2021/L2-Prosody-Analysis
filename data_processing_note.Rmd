...
```
## Part 1. Processing PELIC_speech_compiled.csv
...
The output of the Part 1 include two files:  
...
### 2.1. Using readtextgrid package: do not use this section. This did not work. 
```{r}
...
```
### 3.2 Add proficiency level information from PELIC csv to praat output 
```{r}
...
### 3.5 Add [normedPhoneDur]. will be used specifically for VOT to account for speech rate.  
```{r}
dat4 <- dat3 %>%
  mutate(normedPhoneDur = PhoneDuration/SyllDuration )
```

### 3.6 Remove numbers from [PrecedingPhone].[PhoneLabel].[FollowingPhone]
...
### 3.7 Add [DIS] and create firstSyll data
```{r}
firstSyll_dat <- dat5 %>%
...
### 3.8 Add [DFL] and create finSyll data
```{r}
finSyll_dat <- dat5 %>%
  group_by(WordLabel,SyllDuration) %>%
...
Here we test if vowel and consonant durations in CV syllable show linear relationships with pause durationa. 
```{r}
...
### 4.2. building models for DIS on CV 
```{r}
...
### 4.3. building models for DFL on CV
```{r}
lm3 = lmer(PhoneDuration~DFL+stress+level_id+(1|WordLabel)+(1|Filename), data=cv_fin_V)
summary(lm3)
lm4 = lmer(normedPhoneDur~DFL+stress+level_id+(1|WordLabel)+(1|Filename), data=cv_fin_C)
summary(lm4)
lm4.2 = lmer(normedPhoneDur~DFL+stress+level_id+(1|WordLabel)+(1|Filename)+(1|PhoneLabel), data=cv_fin_C)
summary(lm4.2)
anova(lm4,lm4.2)

## Adding interaction because I am interested in how proficiency level affects stress and boundary effects.
lm5 = lmer(PhoneDuration~DFL*stress*level_id+(1|WordLabel)+(1|Filename), data=cv_fin_V)
summary(lm5)
lm6 = lmer(normedPhoneDur~DFL*stress*level_id+(1|WordLabel)+(1|Filename), data=cv_fin_C)
summary(lm6)
```
...
```{r} 
ggplot(cv_ini_V, aes(as.factor(DIS),normedPhoneDur, color=stress))+
  geom_boxplot(outlier.shape=NA)+
  xlab('Boundary Degree 0<1<2<3')+
  ggtitle("DIS effects on vowel in #CV")+
...
### 6. Separate CVC syllable structure 
...
```{r}
lm11 = lmer(PhoneDuration~DIS+stress+level_id+(1|WordLabel)+(1|Filename), data=cvc_ini_V)
summary(lm11)
...
```{r}
ggplot(vowel_dat_DIS,
       aes(as.factor(DIS),normedPhoneDur, color=as.factor(level_id))) + stat_summary(fun.data=mean_cl_boot)+ xlab('Boundary Degree 0<1<2<3')+
  ggtitle("DIS effects on vowel")+facet_wrap(~PrimaryStress) #+ ylim (0,0.4)
```