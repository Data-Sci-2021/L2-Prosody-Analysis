...
## 4. Statistics
### Domain initial strenthening - use <firstSyllDat> data to examine domain initial pause effects
### 4.1. Filter out the syllable structures that we will examine
```{r}
target <- c("CVC","CVCC","CV")

firstSyll_dat1 <- firstSyll_dat %>% 
  filter(SyllCV %in% target) %>%
   drop_na(level_id)
```

### 4.2. Prepare data for DIS on vot
```{r}
vot <- c("t_vot","k_vot","p_vot")

vot_dat_DIS <- firstSyll_dat1 %>% 
  filter(PhoneLabel %in% vot)

```

### 4.3. Prepare data for DIS on vowel
```{r}
vowel <- c("V")

vowel_dat_DIS <- firstSyll_dat1 %>% 
  filter(PhoneLabel %in% vowel)
```
...
ggplot(vot_dat_DIS,
  ...
  ggtitle("DIS effects on VOT")+facet_wrap(~PrimaryStress) #+ ylim (0,0.4)
```
```{r}
ggplot(vot_dat_DIS,
       aes(as.factor(DIS),PhoneDuration, color=as.factor(level_id))) + stat_summary(fun.data=mean_cl_boot)+ xlab('Boundary Degree 0<1<2<3')+
  ggtitle("DIS effects on VOT")+facet_wrap(~PrimaryStress) #+ ylim (0,0.4)
```
